...
//!
//! All scientific orchestration lives in [`sardine::run`]; this binary
...
/// Thin CLI wrapper around [`sardine::run::parse_iw_selection`].
...
fn parse_iw_selection(iw: &str, burst_range: Option<&str>) -> Result<IwSelection> {
    sardine::run::parse_iw_selection(iw, burst_range)
        .with_context(|| format!("--iw / --burst-range: invalid selection (iw={iw:?})"))
}

/// Parse `--resampling bilinear|bicubic|lanczos3`.
fn parse_resampling(s: &str) -> Result<sardine::run::ResamplingKernel> {
    use sardine::run::ResamplingKernel;
...
impl ProcessArgs {
    fn into_options(mut self) -> Result<ProcessOptions> {
        ...
        let speckle_order = sardine::run::parse_speckle_order(&self.speckle_order)
            .with_context(|| "--speckle-order")?;

        let mode = sardine::run::parse_output_mode(&self.mode)
            .with_context(|| "--mode")?;
...
fn cmd_process(args: ProcessArgs) -> Result<()> {
    sardine::run::run_process_multi(&args.into_options()?)
}
...
fn cmd_search_slc(args: SearchSlcArgs) -> Result<()> {
    #[cfg(feature = "slc-fetch")]
    {
        use sardine::slc_fetch::{search_slc, SlcSearchParams};
...
fn cmd_grd(args: GrdArgs) -> Result<()> {
    sardine::run::run_grd_multi(&args.into_options()?)
}
...
struct InsarArgs {
    ...
    #[arg(long, value_name = "N", default_value_t = sardine::insar::interferogram::DEFAULT_COH_AZ_LOOKS)]
    az_looks: usize,

    /// Range window size (samples) for coherence estimation.
    #[arg(long, value_name = "N", default_value_t = sardine::insar::interferogram::DEFAULT_COH_RG_LOOKS)]
    rg_looks: usize,
...
fn cmd_batch(args: BatchArgs) -> Result<()> {
    ...
    let results = sardine::run::run_process_batch(&opts);
...
fn main() -> ExitCode {
    ...
    //   RUST_LOG=sardine=debug sardine process …
