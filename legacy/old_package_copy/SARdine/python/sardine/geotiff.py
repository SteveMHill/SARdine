...
def export_geotiff(
    ...
    # FIX 1.3: Set nodata value if provided (critical for masking gaps/NaN pixels)
    if nodata is not None:
        profile['nodata'] = nodata
    elif np.issubdtype(data.dtype, np.floating) and np.isnan(data).any():
        # Auto-detect NaN in float data and set nodata accordingly
