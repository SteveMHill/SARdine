...
<s1:product xmlns:s1="s1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <generalAnnotation>
    ...
    <azimuthFmRateList>
      <azimuthFmRate>
        <azimuthTime>2020-12-28T21:59:42.123456Z</azimuthTime>
        <t0>0.0</t0>
        <azimuthFmRatePolynomial>100.0 0.1 -0.001</azimuthFmRatePolynomial>
      </azimuthFmRate>
      <azimuthFmRate>
        <azimuthTime>2020-12-28T21:59:52Z</azimuthTime>
        <t0>0.5</t0>
        <azimuthFmRatePolynomial>101.0 0.05 -0.0005</azimuthFmRatePolynomial>
      </azimuthFmRate>
    </azimuthFmRateList>
  </generalAnnotation>
  ...
  <imageAnnotation>
    ...
    <processingInformation>
      <swathProcParamsList>
        <swathProcParams><swath>IW1</swath><burstDuration>0.5</burstDuration></swathProcParams>
        <swathProcParams><swath>IW2</swath><burstDuration>0.5</burstDuration></swathProcParams>
        <swathProcParams><swath>IW3</swath><burstDuration>0.5</burstDuration></swathProcParams>
      </swathProcParamsList>
    </processingInformation>
  </imageAnnotation>
  ...
  <generalAnnotation>
    ...
    <azimuthFmRateList>
      <azimuthFmRate>
        <azimuthTime>2020-12-28T21:59:42Z</azimuthTime>
        <t0>0.0</t0>
        <azimuthFmRatePolynomial>95.0 0.2 -0.001</azimuthFmRatePolynomial>
      </azimuthFmRate>
    </azimuthFmRateList>
  </generalAnnotation>
...
"#;
    ...
    #[test]
    fn rejects_invalid_zero_bbox() {
        let xml = r#"
<product>
  <geolocationGrid>
    <geolocationGridPointList>
      <geolocationGridPoint>
        <slantRangeTime>0.0</slantRangeTime>
        <line>0</line><pixel>0</pixel>
        <latitude>0</latitude><longitude>0</longitude><height>0</height>
        <incidenceAngle>0</incidenceAngle><elevationAngle>0</elevationAngle>
      </geolocationGridPoint>
      <geolocationGridPoint>
        <slantRangeTime>0.0</slantRangeTime>
        <line>1</line><pixel>1</pixel>
...
"#;
        ...
        let parsed = parse_annotation_xml(xml).expect("parse minimal xml");
        let bbox = sardine::io::annotation::ProductRoot::extract_bounding_box(&parsed);
        assert!(bbox.is_err(), "Zeroed bounding box should be rejected");
    }

    #[test]
    fn antenna_pattern_space_separated_arrays() {
        let parsed = parse_annotation_xml(XML_SAFE).expect("parse");
        let sets = parsed.get_antenna_patterns().expect("antenna pattern sets");
    ...
    fn range_doppler_params_are_consistent() {
        ...
        let params = parsed
            .extract_range_doppler_params(&orbit_vectors)
            .expect("rd params");
