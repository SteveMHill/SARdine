...
impl DemReader {
    /// Helper function to compute pixel window for bounding box with robust y-index math
    /// Handles negative pixel_height correctly
    fn pixel_window_for_bbox(
        gt: [f64; 6],
        width: usize,
        height: usize,
        bbox: &BoundingBox,
        ...
        // No rotation assumed
        let world_to_pix = |x: f64, y: f64| -> (f64, f64) {
            let col = (x - gt[0]) / gt[1];
            let row = (y - gt[3]) / gt[5]; // works for negative pixel_height too
            (col, row)
        };

        let (c0, r0) = world_to_pix(bbox.min_lon, bbox.max_lat);
        let (c1, r1) = world_to_pix(bbox.max_lon, bbox.min_lat);

        let cmin = c0.min(c1).floor().max(0.0) as isize;
        let cmax = c0.max(c1).ceil().min((width - 1) as f64) as isize;
        let rmin = r0.min(r1).floor().max(0.0) as isize;
        let rmax = r0.max(r1).ceil().min((height - 1) as f64) as isize;
        ...
        Some((
            cmin as usize,
            rmin as usize,
            (cmax - cmin + 1) as usize,
            (rmax - rmin + 1) as usize,
        ))
    }
    ...
    pub fn read_dem<P: AsRef<Path>>(
        ...
        log::debug!("DEM size: {}x{}", w, h);
        log::debug!("DEM geotransform: {:?}", gt);

        // Use robust pixel window calculation
        let (px, py, pw, ph) = Self::pixel_window_for_bbox(gt, w, h, bbox).ok_or_else(|| {
            SarError::Processing(format!(
                "Requested bounding box {:?} does not overlap with DEM coverage",
                bbox
            ))
        })?;

        log::debug!(
            "Clipping DEM to pixel bounds: x={}..{}, y={}..{}",
            px,
            py,
            px + pw,
            py + ph
        );
        log::debug!("Clipped size: {}x{}", pw, ph);

        // Read elevation data from first band for the clipped region
        let rasterband = dataset.rasterband(1)?;
        let band_data =
            rasterband.read_as::<f32>((px as isize, py as isize), (pw, ph), (pw, ph), None)?;

        // Convert to ndarray
        let dem_array = Array2::from_shape_vec((ph, pw), band_data.data)
        ...
        let geo_transform_struct = GeoTransform {
            top_left_x: gt[0] + (px as f64) * gt[1],
            top_left_y: gt[3] + (py as f64) * gt[5],
            pixel_width: gt[1],
            pixel_height: gt[5],
            rotation_x: gt[2],
            rotation_y: gt[4],
        ...
        log::debug!(
            "Clipped DEM bounds: X={:.6} to {:.6}, Y={:.6} to {:.6}",
            geo_transform_struct.top_left_x,
            geo_transform_struct.top_left_x + (pw as f64) * gt[1],
            geo_transform_struct.top_left_y + (ph as f64) * gt[5],
            geo_transform_struct.top_left_y
        );

        Ok((dem_array, geo_transform_struct))
    }

    /// Extract DEM pixel spacing in meters
    ///
    /// Returns (dx_meters, dy_meters) where dx/dy are pixel spacings in meters.
    /// For projected CRS (meters): uses geotransform directly
    /// For geographic CRS (degrees): converts using latitude-dependent approximation
    ///
    /// # Arguments
    /// * `dem_path` - Path to the DEM file
    /// * `bbox` - Optional bounding box for lat-dependent degree-to-meter conversion
    ///
    /// # Returns
    /// * `Ok((dx, dy))` - Pixel spacing in meters
    /// * `Err` - If DEM cannot be opened or CRS is invalid
    pub fn get_dem_pixel_spacing_meters<P: AsRef<Path>>(
        dem_path: P,
        ...
        // Extract pixel sizes from geotransform
        let dx = gt[1].abs(); // pixel width
        let dy = gt[5].abs(); // pixel height (usually negative, so abs)

        log::debug!("DEM geotransform: {:?}", gt);
        log::debug!("Raw pixel spacing: dx={:.8}, dy={:.8}", dx, dy);

        // Check if CRS is projected (meters) or geographic (degrees)
        if spatial_ref.is_projected() {
            // Projected CRS - dx/dy should already be in meters
            log::info!(
                "DEM CRS is projected - pixel spacing: dx={:.2}m, dy={:.2}m",
                dx,
                dy
            );

            // Sanity check: typical DEM resolutions are 1-200 meters
            if dx < 0.1 || dx > 500.0 || dy < 0.1 || dy > 500.0 {
                log::warn!(
                    "DEM pixel spacing seems unusual for projected CRS: dx={:.2}m, dy={:.2}m",
                    dx,
                    dy
                );
                log::warn!("Expected range: 1-200m for most DEMs. Verify DEM CRS is correct.");
            }

            Ok((dx, dy))
        } else {
            // Geographic CRS - dx/dy are in degrees, need conversion to meters
            log::info!("DEM CRS is geographic (degrees) - converting to meters");
            log::debug!("Pixel spacing in degrees: dx={:.8}°, dy={:.8}°", dx, dy);

            // Use center latitude for degree-to-meter conversion
            let center_lat = if let Some(bbox) = bbox {
                (bbox.min_lat + bbox.max_lat) / 2.0
            } else {
                // Use DEM center
                let bounds = dataset.raster_size();
                let center_x = gt[0] + (bounds.0 as f64 / 2.0) * gt[1];
                let center_y = gt[3] + (bounds.1 as f64 / 2.0) * gt[5];
                center_y
            };

            log::debug!("Using center latitude: {:.6}°", center_lat);

            // Convert degrees to meters using WGS84 latitude-dependent scaling
            // At latitude φ:
            //   meters_per_degree_lon = N(φ) * cos(φ) * π/180
            //   meters_per_degree_lat = M(φ) * π/180
            // where N(φ) is the radius of curvature in the prime vertical and
            //       M(φ) is the meridional radius of curvature
            ...
            let dx_meters = dx * meters_per_degree_lon;
            let dy_meters = dy * meters_per_degree_lat;

            log::info!(
                "DEM pixel spacing in meters: dx={:.2}m, dy={:.2}m (at lat {:.2}°)",
                dx_meters,
                dy_meters,
                center_lat
            );

            // Sanity check for geographic DEMs
            if dx_meters < 1.0 || dx_meters > 5000.0 || dy_meters < 1.0 || dy_meters > 5000.0 {
                log::warn!(
                    "Converted DEM pixel spacing seems unusual: dx={:.2}m, dy={:.2}m",
                    dx_meters,
                    dy_meters
                );
                log::warn!("Original degrees: dx={:.8}°, dy={:.8}°", dx, dy);
            }

            Ok((dx_meters, dy_meters))
        }
    }

    /// Get DEM pixel spacing with fallback values for common DEM types
    ///
    /// Get DEM pixel spacing with filename-based inference for standard DEMs.
    ///
    /// **Note:** In strict mode (default), this function will panic if DEM metadata
    /// cannot be extracted. Set `SARDINE_STRICT=0` to enable filename-based fallbacks.
    ///
    /// Fallback resolutions (only used when SARDINE_STRICT=0):
    /// - SRTM 1": ~30m
    /// - SRTM 3": ~90m
    /// - Copernicus DEM 30m: 30m
    /// - ASTER GDEM: ~30m
    pub fn get_dem_pixel_spacing_with_fallback<P: AsRef<Path>>(
        dem_path: P,
        bbox: Option<&BoundingBox>,
    ) -> (f64, f64) {
        // SCIENTIFIC MODE (default): Require DEM metadata - no heuristic fallbacks
        // This ensures accurate terrain correction calculations
        if crate::types::strict_mode() {
            match Self::get_dem_pixel_spacing_meters(&dem_path, bbox) {
                Ok((dx, dy)) => return (dx, dy),
                Err(e) => panic!(
                    "DEM pixel spacing extraction failed: {}. \n\
                    Scientific mode requires accurate DEM metadata. \n\
                    Solutions: \n\
                    1. Use a DEM with proper GeoTIFF metadata (recommended: Copernicus DEM) \n\
                    2. Set SARDINE_STRICT=0 to enable filename-based fallbacks (not recommended for research)",
                    e
                ),
            }
        }
        match Self::get_dem_pixel_spacing_meters(&dem_path, bbox) {
            Ok((dx, dy)) => {
                log::info!(
                    "✅ Successfully detected DEM pixel spacing: dx={:.2}m, dy={:.2}m",
                    dx,
                    dy
                );
                (dx, dy)
            ...
            Err(e) => {
                log::warn!("⚠️ Could not detect DEM pixel spacing: {}", e);

                // Try to infer from filename or provide safe defaults
                let filename = dem_path
                    .as_ref()
                    .file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("")
                ...
                let (dx, dy) = if filename.contains("srtm") && filename.contains("3") {
                    log::info!("📍 Detected SRTM 3\" from filename - using 90m spacing");
                    (90.0, 90.0)
                } else if filename.contains("srtm") || filename.contains("hgt") {
                    log::info!("📍 Detected SRTM 1\" from filename - using 30m spacing");
                    (30.0, 30.0)
                } else if filename.contains("copernicus") || filename.contains("cop30") {
                    log::info!("📍 Detected Copernicus DEM 30m from filename - using 30m spacing");
                    (30.0, 30.0)
                } else if filename.contains("aster") {
                    log::info!("📍 Detected ASTER GDEM from filename - using 30m spacing");
                    (30.0, 30.0)
                } else {
                    log::warn!(
                        "📍 Unknown DEM type - using default 30m spacing (suitable for most DEMs)"
                    );
                    log::warn!("    For best results, ensure DEM has proper CRS metadata or specify spacing manually");
                    (30.0, 30.0)
                };

                log::info!(
                    "🔧 Using fallback DEM pixel spacing: dx={:.1}m, dy={:.1}m",
                    dx,
                    dy
                );
                (dx, dy)
    ...
    pub fn download_srtm_tiles(bbox: &BoundingBox, output_dir: &str) -> SarResult<Vec<String>> {
        ...
        std::fs::create_dir_all(output_dir).map_err(|e| SarError::Io(e))?;
        ...
        for tile in tiles {
            ...
            if download_start.elapsed() > download_timeout {
                log::warn!(
                    "Download process timed out after {} seconds",
            ...
            for ext in &["hgt", "tif", "tiff"] {
                let test_path = format!("{}/{}.{}", output_dir, tile, ext);
                if std::path::Path::new(&test_path).exists() {
                    log::info!("SRTM tile {} already exists, skipping download", test_path);
            ...
            if let Some(path) = Self::try_download_from_sources(&tile, output_dir)? {
                log::info!(
                    "Successfully downloaded {} in {:.1}s",
                    path,
        ...
        if downloaded_files.is_empty() {
            return Err(SarError::Processing(
                "Failed to download any SRTM tiles from available sources.\n\
                 Please check internet connection or provide DEM files manually."
                    .to_string(),
            ));
        }
    ...
    fn try_download_from_sources(tile: &str, output_dir: &str) -> SarResult<Option<String>> {
        ...
        for (i, url) in sources.iter().enumerate() {
            log::info!("Attempting source {} of {}: {}", i + 1, sources.len(), url);

            match Self::download_and_extract_srtm_tile(url, output_dir, tile) {
                Ok(path) => {
                    log::info!("Successfully downloaded from source {}: {}", i + 1, path);
                    return Ok(Some(path));
                }
                Err(e) => {
        ...
        log::info!("2. Download tile {} and save in {}", tile, output_dir);
    ...
    /// Uses inclusive boundary calculation to prevent missing edge tiles
    fn calculate_srtm_tiles(bbox: &BoundingBox) -> Vec<String> {
        let mut tiles = Vec::new();

        // Debug: Log the bounding box values
        log::info!(
            "calculate_srtm_tiles input bbox: min_lat={}, max_lat={}, min_lon={}, max_lon={}",
            bbox.min_lat,
            bbox.max_lat,
            bbox.min_lon,
            bbox.max_lon
        ...
        // SRTM tiles are 1x1 degree with inclusive boundary handling
        let min_lat = bbox.min_lat.floor() as i32;
        let max_lat = ((bbox.max_lat - 1e-12).floor()) as i32; // Subtract small epsilon for boundary
        let min_lon = bbox.min_lon.floor() as i32;
        let max_lon = ((bbox.max_lon - 1e-12).floor()) as i32; // Subtract small epsilon for boundary

        log::info!(
            "Tile ranges: lat {} to {}, lon {} to {}",
            min_lat,
            max_lat,
            min_lon,
            max_lon
    ...
    fn download_and_extract_srtm_tile(
        url: &str,
        output_dir: &str,
        tile: &str,
    ) -> SarResult<String> {
        ...
        let response = client
            ...
            .map_err(|e| SarError::Processing(format!("HTTP error for {}: {}", url, e)))?;

        let bytes = response
            .bytes()
            .map_err(|e| SarError::Processing(format!("Read body: {}", e)))?;
        ...
        std::fs::create_dir_all(output_dir).map_err(SarError::Io)?;

        // Handle different file formats based on URL and content
        if url.ends_with(".hgt.gz") || Self::is_gzip_content(&bytes) {
            let out = format!("{}/{}.hgt", output_dir, tile);
            Self::extract_gzipped_hgt(&bytes, &out)?;
            return Ok(out);
        } else if url.ends_with(".zip") || Self::is_zip_content(&bytes) {
            let out = format!("{}/{}.hgt", output_dir, tile);
            Self::extract_srtm_zip(&bytes, &out)?;
            return Ok(out);
        } else if url.ends_with(".tif") || url.ends_with(".tiff") {
            // Don't rename GeoTIFFs to .hgt - return the actual saved path
            let out = format!("{}/{}.tif", output_dir, tile);
            std::fs::write(&out, bytes).map_err(SarError::Io)?;
            return Ok(out);
        } else {
            // Direct HGT file
            let out = format!("{}/{}.hgt", output_dir, tile);
    ...
    fn extract_srtm_zip(zip_data: &[u8], output_path: &str) -> SarResult<()> {
        ...
        for i in 0..archive.len() {
            ...
            if file.name().ends_with(".hgt") {
                ...
                std::io::copy(&mut file, &mut buffer).map_err(|e| SarError::Io(e))?;

                std::fs::write(output_path, buffer).map_err(|e| SarError::Io(e))?;
        ...
        Err(SarError::Processing(
            "No HGT file found in ZIP archive".to_string(),
        ))
    }
    ...
    fn extract_gzipped_hgt(gzip_data: &[u8], output_path: &str) -> SarResult<()> {
        ...
        if decompressed.is_empty() {
            return Err(SarError::Processing(
                "Decompressed HGT file is empty".to_string(),
            ));
        }
        ...
        std::fs::write(output_path, decompressed).map_err(|e| SarError::Io(e))?;
    ...
    pub fn prepare_dem_for_scene(
        ...
        std::fs::create_dir_all(&srtm_cache_dir).map_err(|e| SarError::Io(e))?;
        ...
        let (dem_array, dem_transform) =
            Self::create_dem_mosaic(&tile_files, bbox, output_resolution)?;

        // Check for excessive upsampling (Issue C.1: DEM upsampling warning)
        // SRTM/Copernicus DEMs are typically ~30m, warn if output resolution is much finer
        // Convert DEM pixel size to approximate meters at scene center latitude
        let center_lat = (bbox.min_lat + bbox.max_lat) / 2.0;
        let dem_pixel_deg = dem_transform.pixel_width.abs();
        let meters_per_deg = 111320.0 * center_lat.to_radians().cos(); // Approximate meters/degree at this latitude
        let dem_resolution_m = dem_pixel_deg * meters_per_deg;

        // Warn if upsampling by more than 2x (C.1 threshold lowered from 4x to 2x)
        let upsample_ratio = dem_resolution_m / output_resolution;
        if upsample_ratio > 2.0 {
            log::warn!(
                "⚠️ DEM upsampling warning: DEM native resolution ({:.1}m) is much coarser than \
                 requested output ({:.1}m). Upsampling ratio: {:.1}x. This does NOT improve \
                 terrain correction accuracy - consider using output_resolution >= {:.1}m.",
                dem_resolution_m,
                output_resolution,
                upsample_ratio,
            ...
            log::debug!(
                "DEM resolution check: native={:.1}m, output={:.1}m, ratio={:.2}x (OK)",
                dem_resolution_m,
                output_resolution,
                upsample_ratio
            );
    ...
    fn find_existing_dem_files(cache_dir: &str, bbox: &BoundingBox) -> SarResult<Vec<String>> {
        ...
        log::info!(
            "Found {} overlapping DEM files in {} and subdirectories",
            dem_files.len(),
            cache_dir
        );
    ...
    fn create_dem_mosaic(
        ...
        log::info!(
            "Creating mosaic from {} SRTM tiles for bbox {:?}",
            tile_files.len(),
            bbox
        );
        ...
        for tile_file in tile_files {
            ...
            if !overlaps {
                log::debug!(
                    "Tile {} does not overlap with requested bbox, skipping",
                    tile_file
                );
                continue;
            }
            ...
            log::debug!(
                "Reading tile {} with intersection bbox {:?}",
                tile_file,
                intersect_bbox
            );
            ...
            match Self::read_dem_safe(tile_file, Some(&intersect_bbox)) {
                Ok((tile_dem, tile_transform)) => {
                    ...
                    let tile_max_lon = tile_transform.top_left_x
                        + (tile_width as f64) * tile_transform.pixel_width;
                    let tile_min_lat = tile_transform.top_left_y
                        + (tile_height as f64) * tile_transform.pixel_height;
                    ...
                    log::debug!(
                        "Successfully read tile {} with dimensions {}x{}",
                        tile_file,
                        tile_height,
                        tile_width
                    );
        ...
        if tile_data.is_empty() {
            return Err(SarError::Processing(
                "No valid DEM tiles could be read".to_string(),
            ));
        }

        if tile_data.len() == 1 {
            // Only one valid tile
            // SAFE: We've already checked that tile_data.len() == 1, so next() will always return Some
            return Ok(tile_data
                .into_iter()
                .next()
                .expect("tile_data should have exactly one element"));
        }

        // SCIENTIFIC MODE: Implement proper DEM mosaicking for multiple tiles
        log::info!(
            "🧩 Creating scientifically valid DEM mosaic from {} tiles",
            tile_data.len()
        );
        ...
        let mosaic_height =
            ((overall_max_lat - overall_min_lat) / pixel_size.abs()).ceil() as usize;

        log::info!(
            "📊 Mosaic dimensions: {}x{} ({}°x{}°)",
            mosaic_width,
            mosaic_height,
        ...
        for (tile_dem, tile_transform) in &tile_data {
            ...
            let tile_start_x =
                ((tile_transform.top_left_x - overall_min_lon) / pixel_size).round() as usize;
            let tile_start_y =
                ((overall_max_lat - tile_transform.top_left_y) / pixel_size.abs()).round() as usize;

            // Copy tile data to mosaic with bounds checking and water masking
            for i in 0..tile_height {
                for j in 0..tile_width {
                    ...
                    if mosaic_y < mosaic_height && mosaic_x < mosaic_width {
                        ...
                        if tile_value != -32768.0 {
                            // Valid elevation data - apply water masking for ocean/shallow water
                            // Clamp bathymetry and shallow water (<5m) to sea level to prevent
                            // Range-Doppler solver failures over ocean pixels
                            let masked_value = if tile_value < 5.0 {
                                // Ocean, bathymetry, or very shallow water -> use sea level
                                0.0
                            } else {
                                tile_value
                            };
                            mosaic[[mosaic_y, mosaic_x]] = masked_value;
                        }
                    }
                }
            }
    ...
    pub fn resample_dem(
        ...
        for i in 0..target_height {
            for j in 0..target_width {
                ...
                let target_x =
                    target_transform.top_left_x + (j as f64) * target_transform.pixel_width;
                let target_y =
                    target_transform.top_left_y + (i as f64) * target_transform.pixel_height;

                // Convert geographic coordinates to source pixel coordinates
                let source_col =
                    (target_x - source_transform.top_left_x) / source_transform.pixel_width;
                let source_row =
                    (target_y - source_transform.top_left_y) / source_transform.pixel_height;

                // Bilinear interpolation with enhanced bounds checking
                if source_col >= 0.0
                    && source_col < (source_width - 1) as f64
                    && source_row >= 0.0
                    && source_row < (source_height - 1) as f64
                {
    ...
    pub fn calculate_slope_aspect(
        ...
        for i in 1..height - 1 {
            for j in 1..width - 1 {
                ...
                if i >= 1 && i < height - 1 && j >= 1 && j < width - 1 {
                    // Validate pixel spacing before division (prevents Inf/NaN)
                    ...
                    aspect[[i, j]] = (std::f32::consts::PI / 2.0 - aspect_raw
        ...
        log::info!(
            "Slope/aspect calculation: {} valid interior pixels, {} NaN input pixels",
            valid_count,
            nan_input_count
        );
    ...
    pub fn slope_aspect_to_normals(slope: &Array2<f32>, aspect: &Array2<f32>) -> Array2<[f32; 3]> {
        ...
        log::info!(
            "Surface normal calculation: {} valid, {} NaN, {} zero-slope pixels",
            valid_count,
            nan_count,
            zero_slope_count
        );
    ...
    fn fill_edge_values(array: &mut Array2<f32>) -> SarResult<()> {
        ...
        if height < 3 || width < 3 {
            return Err(SarError::Processing(
                "Array too small for edge filling".to_string(),
            ));
        }
    ...
    pub fn test_srtm_download(tile_name: &str, output_dir: &str) -> SarResult<String> {
        ...
        // Check if file already exists (try common extensions)
        for ext in &["hgt", "tif", "tiff"] {
            let test_path = format!("{}/{}.{}", output_dir, tile_name, ext);
            if std::path::Path::new(&test_path).exists() {
                log::info!("SRTM tile {} already exists at: {}", tile_name, test_path);
                return Ok(test_path);
            }
        }

        // Try downloading from sources
        if let Some(path) = Self::try_download_from_sources(tile_name, output_dir)? {
            return Ok(path);
        }

        Err(SarError::Processing(format!(
            "Failed to download SRTM tile {} from all available sources",
            tile_name
        )))
    }
    ...
    pub fn fill_dem_voids(dem: &mut Array2<f32>, no_data_value: f32) -> SarResult<()> {
        ...
        log::info!(
            "Found {} void pixels in DEM ({:.2}%)",
            void_count,
            (void_count as f64 / (height * width) as f64) * 100.0
        );
            ...
            for i in 1..height - 1 {
                for j in 1..width - 1 {
                    if void_mask[[i, j]] {
                        ...
                        if count >= 3 {
                            // Need at least 3 valid neighbors
    ...
    pub fn create_terrain_mask(
        ...
        log::debug!(
            "Creating terrain mask with incidence angle range: {:.1}° - {:.1}°",
            min_angle_deg,
            max_angle_deg
        );
        ...
        for i in 0..height {
            for j in 0..width {
                ...
                if angle.is_nan() || angle < min_angle_rad || angle > max_angle_rad || angle <= 0.0
                {
                    mask[[i, j]] = false;
                    masked_count += 1;
                }
        ...
        log::info!(
            "Masked {} pixels ({:.2}%) due to invalid incidence angles",
            masked_count,
            mask_percentage
        );
    ...
    pub fn complete_terrain_flattening_pipeline(
        ...
        let (mut dem, dem_geo_transform) =
            Self::prepare_dem_for_scene(sar_bbox, output_resolution, cache_dir)?;
        ...
        let dem_bbox = BoundingBox {
            min_lat: dem_geo_transform.top_left_y
                + dem_height * dem_geo_transform.pixel_height.min(0.0),
            max_lat: dem_geo_transform.top_left_y
                + dem_height * dem_geo_transform.pixel_height.max(0.0),
            min_lon: dem_geo_transform.top_left_x
                + dem_width * dem_geo_transform.pixel_width.min(0.0),
            max_lon: dem_geo_transform.top_left_x
                + dem_width * dem_geo_transform.pixel_width.max(0.0),
        };

        log::debug!(
            "DEM bbox: min_lat={:.6}, max_lat={:.6}, min_lon={:.6}, max_lon={:.6}",
            dem_bbox.min_lat,
            dem_bbox.max_lat,
            dem_bbox.min_lon,
            dem_bbox.max_lon
        ...
        log::debug!(
            "SAR bbox: min_lat={:.6}, max_lat={:.6}, min_lon={:.6}, max_lon={:.6}",
            sar_bbox.min_lat,
            sar_bbox.max_lat,
            sar_bbox.min_lon,
            sar_bbox.max_lon
        ...
        log::info!(
            "DEM pixel spacing: {:.8}° x {:.8}°",
            dem_geo_transform.pixel_width,
            dem_geo_transform.pixel_height.abs()
        );
        log::info!(
            "SAR pixel spacing: {:.8}° x {:.8}°",
            sar_geo_transform.pixel_width,
            sar_geo_transform.pixel_height.abs()
        );

        let resampled_dem = if dem_geo_transform.pixel_width.abs()
            != sar_geo_transform.pixel_width.abs()
            || dem_geo_transform.pixel_height.abs() != sar_geo_transform.pixel_height.abs()
        {
            log::info!(
                "Resampling DEM from {:.6}° to match SAR geometry",
                dem_geo_transform.pixel_width
            );
            let resampled =
        ...
        // Step 5: Compute slope and aspect in meters, not degrees
        log::info!("📈 Step 5: Computing terrain slope and aspect in meters");
        let mean_lat = 0.5 * (sar_bbox.min_lat + sar_bbox.max_lat);
        ...
        let dx_m = sar_geo_transform.pixel_width.abs() * m_per_deg_lon;
        let dy_m = sar_geo_transform.pixel_height.abs() * m_per_deg_lat;

        let (slope, aspect) = Self::calculate_slope_aspect(&resampled_dem, (dx_m, dy_m))?;
        log::info!(
            "Slope range: {:.4} to {:.4}",
        ...
        // Step 6: Compute surface normal vectors in meters
        log::info!("🔺 Step 6: Computing surface normal vectors");
        let surface_normals = Self::compute_precise_surface_normals_xy(&resampled_dem, dx_m, dy_m)?;
        log::info!(
            "Surface normals computed for {}x{} pixels",
            resampled_dem.nrows(),
            resampled_dem.ncols()
        );
        ...
        let look_vector = (
            ...
            std::f32::consts::FRAC_1_SQRT_2,
        ); // Simplified look vector
        let local_incidence_angles =
            Self::compute_precise_local_incidence_angles(&surface_normals, &look_vector)?;
        log::info!(
            "Incidence angles range: {:.2}° to {:.2}°",
        ...
        use crate::core::masking::advanced_masking::{
        ...
        let masking_result = masking_processor
            .process_advanced_masking(
                sar_image,
        ...
        log::info!(
            "Advanced masking completed with {:.1}% confidence",
            masking_result.quality_metrics.overall_quality * 100.0
        );
        log::info!(
            "Terrain mask: {}/{} pixels valid ({:.1}%)",
            valid_mask_count,
            terrain_mask.len(),
        ...
        log::info!(
            "  - Water pixels: {}",
            masking_result.statistics.water_pixels
        );
        log::info!(
            "  - Layover pixels: {}",
            masking_result.statistics.layover_pixels
        );
        log::info!(
            "  - Noise pixels: {}",
            masking_result.statistics.noise_pixels
        );
        log::info!(
            "  - Edge artifacts: {}",
            masking_result.statistics.edge_pixels
        );
        log::info!(
            "  - Statistical outliers: {}",
            masking_result.statistics.outlier_pixels
        );
        log::info!(
            "  - Data quality score: {:.3}",
            masking_result.statistics.data_quality_score
        );
        ...
        // DEBUG: Check input range for extreme values
        let input_min = flattened_image.iter().fold(f32::INFINITY, |a, &b| a.min(b));
        let input_max = flattened_image
            .iter()
            .fold(f32::NEG_INFINITY, |a, &b| a.max(b));
        log::warn!(
            "🔍 DEBUG: Input image range: {:.6} to {:.6}",
            input_min,
            input_max
        );

        // Check for extreme values that would cause the 215 billion issue
        if input_max > 1e9 {
            log::error!("⚠️  EXTREME INPUT VALUES DETECTED: max={:.6e}", input_max);
            log::error!("   This explains the 215 billion range values!");

            // Count extreme values
            let extreme_count = flattened_image.iter().filter(|&&x| x.abs() > 1e9).count();
            log::error!(
                "   Extreme values (>1e9): {} of {}",
                extreme_count,
                flattened_image.len()
            );
        }
        ...
        log::info!(
            "Flattened image NaN count: {}",
            flattened_image.iter().filter(|&&x| x.is_nan()).count()
        );
    ...
    pub fn complete_advanced_terrain_correction_pipeline(
        ...
        let (geocoded_image, _final_geo_transform) = if correction_type == "geometric"
            || correction_type == "both"
        {
            ...
            let geocoded_coordinates =
                Self::solve_range_doppler_equations(&dem_ecef, orbit_data, wavelength)?;

            // Map SAR image to DEM grid using computed coordinates
            let geocoded =
                Self::map_sar_to_dem_grid(sar_image, &geocoded_coordinates, &dem_geo_transform)?;
        ...
        let surface_normals = Self::compute_precise_surface_normals(&smoothed_dem, pixel_spacing)?;
        ...
        let look_vector = if let Some(state_vector) = orbit_data.state_vectors.first() {
            ...
            (
                vel[0] as f32 / mag as f32,
                vel[1] as f32 / mag as f32,
                vel[2] as f32 / mag as f32,
            )
        } else {
        ...
        let (corrected_image, layover_shadow_mask) = if correction_type == "radiometric"
            || correction_type == "both"
        {
            log::info!("🏔️  Step 5: Advanced Gamma Flattening Radiometric Terrain Correction");

            // Detect layover and shadow regions
            let layover_shadow_mask =
                Self::advanced_layover_shadow_detection(&smoothed_dem, &local_incidence_angles)?;
    ...
    pub fn read_dem_safe<P: AsRef<Path>>(
        ...
        if width < 3 || height < 3 {
            log::warn!(
                "DEM too small ({}x{}), creating minimal valid DEM",
                width,
                height
            );
            let min_size = 10;
    ...
    fn read_srtm_hgt_file_safe<P: AsRef<Path>>(
        ...
        if let Some(bbox) = bbox {
            let (cropped_dem, cropped_transform) =
                Self::crop_dem_to_bbox(&dem_array, &transform, bbox)?;
    ...
    /// Crop DEM array to specific bounding box with robust bounds validation
    fn crop_dem_to_bbox(
        dem: &Array2<f32>,
        tr: &GeoTransform,
        bbox: &BoundingBox,
        ...
        let (px, py, pw, ph) = Self::pixel_window_for_bbox(gt, w, h, bbox)
            .ok_or_else(|| SarError::Processing("Requested bbox doesn't overlap DEM".into()))?;

        let cropped = dem.slice(s![py..py + ph, px..px + pw]).to_owned();
        let new_tr = GeoTransform {
            top_left_x: tr.top_left_x + (px as f64) * tr.pixel_width,
            top_left_y: tr.top_left_y + (py as f64) * tr.pixel_height,
            pixel_width: tr.pixel_width,
            pixel_height: tr.pixel_height,
            rotation_x: tr.rotation_x,
            rotation_y: tr.rotation_y,
        };

        log::debug!("Cropped DEM from {}x{} to {}x{}", h, w, ph, pw);
        Ok((cropped, new_tr))
    }
    ...
    fn parse_hgt_filename(filename: &str) -> SarResult<(i32, i32)> {
        ...
        let lat: i32 = lat_str.parse().map_err(|_| {
            SarError::Processing(format!("Invalid latitude in filename: {}", filename))
        })?;
        let lon: i32 = lon_str.parse().map_err(|_| {
            SarError::Processing(format!("Invalid longitude in filename: {}", filename))
        })?;
    ...
    /// Compute precise surface normals with anisotropic pixel spacing
    /// Handles different x/y resolutions correctly
    fn compute_precise_surface_normals_xy(
        dem: &Array2<f32>,
        dx_m: f64,
        dy_m: f64,
        ...
        let (h, w) = dem.dim();
        if h < 3 || w < 3 {
            return Err(SarError::Processing("DEM too small".into()));
        }
        ...
        for i in 1..h - 1 {
            for j in 1..w - 1 {
                let dzdx = (dem[[i, j + 1]] - dem[[i, j - 1]]) / (2.0 * dx);
                let dzdy = (dem[[i + 1, j]] - dem[[i - 1, j]]) / (2.0 * dy);
                let mag = (dzdx * dzdx + dzdy * dzdy + 1.0).sqrt();
                nx[[i, j]] = -dzdx / mag;
                ny[[i, j]] = -dzdy / mag;
                nz[[i, j]] = 1.0 / mag;
            }
    ...
    fn compute_precise_surface_normals(
        ...
        Self::compute_precise_surface_normals_xy(dem, pixel_spacing, pixel_spacing)
    }
    ...
    fn solve_range_doppler_equations(
        ...
        let mut geocoded_coords = Array2::from_elem((h, w), (0.0, 0.0));

        let _svs = &orbit_data.state_vectors; // placeholder

        for i in 0..h {
            for j in 0..w {
                let x = dem_ecef[[i, j, 0]];
                let y = dem_ecef[[i, j, 1]];
                let z = dem_ecef[[i, j, 2]];
                if x != 0.0 || y != 0.0 || z != 0.0 {
                    let (lon, lat) = Self::ecef_to_wgs84(x, y, z);
                    geocoded_coords[(i, j)] = (lon, lat);
                }
            }
        }
    ...
    fn map_sar_to_dem_grid(
        ...
        for ((i, j), value) in sar_data.indexed_iter() {
            ...
            if lon != 0.0 && lat != 0.0 {
                // Transform geographic coordinates to DEM pixel coordinates
                let pixel_x =
                    ((lon - dem_transform.top_left_x) / dem_transform.pixel_width) as usize;
                let pixel_y =
                    ((dem_transform.top_left_y - lat) / dem_transform.pixel_height.abs()) as usize;
    ...
    fn advanced_layover_shadow_detection(
        ...
        for ((i, j), mask_val) in layover_shadow_mask.indexed_iter_mut() {
            ...
            let is_shadow =
                !is_facing_radar && local_slope > (std::f32::consts::PI / 2.0 - look_angle).sin();
    ...
    fn parse_orbit_state_vectors(orbit_data: &str) -> SarResult<Vec<OrbitStateVector>> {
        ...
        if let Some(start) = orbit_data.find("<orbitList>") {
            if let Some(end) = orbit_data.find("</orbitList>") {
                ...
                for (pos_cap, vel_cap) in pos_regex
                    .captures_iter(orbit_section)
                    .zip(vel_regex.captures_iter(orbit_section))
                {
                    let px: f64 = pos_cap[1].parse().map_err(|e| {
                        log::error!(
                            "🚨 CRITICAL: Failed to parse orbit position X: {} - error: {}",
                            &pos_cap[1],
                            e
                        );
                        SarError::DataProcessingError(format!(
                            "Invalid orbit position X value: {}",
                            &pos_cap[1]
                        ))
                    })?;
                    let py: f64 = pos_cap[2].parse().map_err(|e| {
                        log::error!(
                            "🚨 CRITICAL: Failed to parse orbit position Y: {} - error: {}",
                            &pos_cap[2],
                            e
                        );
                        SarError::DataProcessingError(format!(
                            "Invalid orbit position Y value: {}",
                            &pos_cap[2]
                        ))
                    })?;
                    let pz: f64 = pos_cap[3].parse().map_err(|e| {
                        log::error!(
                            "🚨 CRITICAL: Failed to parse orbit position Z: {} - error: {}",
                            &pos_cap[3],
                            e
                        );
                        SarError::DataProcessingError(format!(
                            "Invalid orbit position Z value: {}",
                            &pos_cap[3]
                        ))
                    })?;
                    let vx: f64 = vel_cap[1].parse().map_err(|e| {
                        log::error!(
                            "🚨 CRITICAL: Failed to parse orbit velocity X: {} - error: {}",
                            &vel_cap[1],
                            e
                        );
                        SarError::DataProcessingError(format!(
                            "Invalid orbit velocity X value: {}",
                            &vel_cap[1]
                        ))
                    })?;
                    let vy: f64 = vel_cap[2].parse().map_err(|e| {
                        log::error!(
                            "🚨 CRITICAL: Failed to parse orbit velocity Y: {} - error: {}",
                            &vel_cap[2],
                            e
                        );
                        SarError::DataProcessingError(format!(
                            "Invalid orbit velocity Y value: {}",
                            &vel_cap[2]
                        ))
                    })?;
                    let vz: f64 = vel_cap[3].parse().map_err(|e| {
                        log::error!(
                            "🚨 CRITICAL: Failed to parse orbit velocity Z: {} - error: {}",
                            &vel_cap[3],
                            e
                        );
                        SarError::DataProcessingError(format!(
                            "Invalid orbit velocity Z value: {}",
                            &vel_cap[3]
                        ))
                    })?;
        ...
        if state_vectors.is_empty() {
            return Err(SarError::Processing(
                "No orbit state vectors found".to_string(),
            ));
        }
