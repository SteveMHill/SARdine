...
impl FastUnitProcessor {
    ...
    /// Conservative unit detection (explicit attribute only)
    fn detect_units(values: &[f64], units_attr: Option<&str>) -> Units {
        // Check explicit unit attribute only; avoid risky heuristics
    ...
    fn convert_f64_db_to_linear(values: &mut [f64]) {
        // Canonical dB to linear: linear = 10^(dB/10)
        // We do not apply sign inversions or tenths-dB heuristics here.
