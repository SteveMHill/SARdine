...
use crate::core::geometry::type_safe_units::{Degrees, Hertz, Meters, Radians, Seconds};
...
pub struct CompactTimingData {
    ...
    /// DC polynomial reference times (seconds since epoch) per burst
    pub dc_t0: Vec<f64>,
    /// FM polynomial reference times (seconds since epoch) per burst
    pub fm_t0: Vec<f64>,
    /// Range-dependent DC polynomial entries (parallel to dc_polynomials when provided)
    pub dc_range_times: Vec<Option<f64>>, // optional slantRangeTime per burst
    /// Range-dependent FM polynomial entries (parallel to fm_polynomials when provided)
    pub fm_range_times: Vec<Option<f64>>,
...
impl SinglePassXmlParser {
    ...
    fn handle_start_element(
        ...
        match tag_name {
            ...
            | "fmRateList"
            | "fmRateEstimate"
            | "dataFmratePolynomial"
            | "dataAzimuthFmRatePolynomial" => {
    ...
    fn parse_and_convert_calibration_values(
        ...
        // SCIENTIFIC FIX: Default scale=1.0 is only valid for linear units
        // For dB units, missing scale is a parsing error
        let scale = match (&units, self.parsing_state.current_scale) {
            (_, Some(s)) => s,
            (UnitType::Linear, None) => 1.0, // Linear values with no scale = identity
            (UnitType::Decibel | UnitType::DecibelTenths, None) => {
                log::warn!(
                    "⚠️  Missing scale factor for {} calibration in dB units; assuming 1.0 (verify calibration XML)",
                    cal_type
                );
                1.0
            }
            (UnitType::Unknown, None) => {
                log::warn!(
                    "⚠️  Missing scale factor for {} calibration with unknown units; assuming 1.0",
                    cal_type
                );
                1.0
            }
    ...
    fn process_timing_element(&mut self, tag_name: &str, text: &str) -> SarResult<()> {
        ...
        // Capture polynomial reference times (t0) for DC/FM if present
        let t0_dc = if tag_name == "t0" && path.contains("dcEstimate") {
            text.trim().parse::<f64>().ok()
        } else {
            None
        };

        let t0_fm = if tag_name == "t0" && path.contains("azimuthFmRate") {
            text.trim().parse::<f64>().ok()
        } else {
            None
        };

        // Capture optional slantRangeTime entries for range-dependent grids
        let slant_range_time = if tag_name == "slantRangeTime" && path.contains("dcEstimate") {
            text.trim().parse::<f64>().ok()
        } else {
            None
        };
        ...
        if let Some(ref mut td) = self.timing_data {
            ...
            if let Some(coeffs) = coeffs_dc {
                if !coeffs.is_empty() {
                    ...
                    log::debug!(
                        "Parsed DC polynomial with {} coefficients",
                        td.dc_polynomials.last().unwrap().len()
                    );
                    // Align t0 and range grids to polynomial index
                    td.dc_t0.push(t0_dc.unwrap_or(f64::NAN));
                    td.dc_range_times.push(slant_range_time);
                }
            }
            ...
            if let Some(coeffs) = coeffs_fm {
                if !coeffs.is_empty() {
                    ...
                    log::debug!(
                        "Parsed FM rate polynomial with {} coefficients",
                        td.fm_polynomials.last().unwrap().len()
                    );
                    td.fm_t0.push(t0_fm.unwrap_or(f64::NAN));
                    td.fm_range_times.push(None); // FM range times are uncommon; filled when present
                }
            }
    ...
    fn finalize_parsing(&mut self) -> SarResult<()> {
        ...
        if let Some(ref mut td) = self.timing_data {
            log::info!(
                ...
                td.num_bursts,
                td.dc_polynomials.len(),
                td.fm_polynomials.len(),
                td.prf_values.len()
            );

            // Hard fail if required polynomials are missing when bursts are known
            if td.num_bursts > 0 {
                if td.dc_polynomials.is_empty() {
                    return Err(SarError::InvalidMetadata(
                        "Missing Doppler centroid polynomials in annotation (dcEstimateList)"
                            .to_string(),
                    ));
                }
                if td.fm_polynomials.is_empty() {
                    return Err(SarError::InvalidMetadata(
                        "Missing azimuth FM rate polynomials in annotation (azimuthFmRate/fmRate)"
                            .to_string(),
                    ));
                }
            }

            // Validate polynomial lengths are consistent to avoid dimension mismatches downstream
            if let Some(first_len) = td.dc_polynomials.first().map(|p| p.len()) {
                if td.dc_polynomials.iter().any(|p| p.len() != first_len) {
                    return Err(SarError::InvalidMetadata(
                        "Inconsistent DC polynomial degrees across bursts".to_string(),
                    ));
                }
            }
            if let Some(first_len) = td.fm_polynomials.first().map(|p| p.len()) {
                if td.fm_polynomials.iter().any(|p| p.len() != first_len) {
                    return Err(SarError::InvalidMetadata(
                        "Inconsistent FM polynomial degrees across bursts".to_string(),
                    ));
                }
            }

            // Validate t0 availability when polynomials are present
            if !td.dc_polynomials.is_empty() {
                if td.dc_t0.len() != td.dc_polynomials.len()
                    || td.dc_t0.iter().any(|v| !v.is_finite())
                {
                    return Err(SarError::InvalidMetadata(
                        "Missing or invalid DC polynomial reference times (t0) in annotation"
                            .to_string(),
                    ));
                }
            }
            ...
            if !td.fm_polynomials.is_empty() {
                if td.fm_t0.len() != td.fm_polynomials.len()
                    || td.fm_t0.iter().any(|v| !v.is_finite())
                {
                    return Err(SarError::InvalidMetadata(
                        "Missing or invalid FM polynomial reference times (t0) in annotation"
                            .to_string(),
                    ));
                }
            }
            ...
            if td.num_bursts > 0 {
                ...
                log::debug!(
                    "Inferred num_bursts={} from DC polynomial count",
                    td.num_bursts
                );
            }
        }
...
impl CompactCalibrationData {
    ...
    pub fn find_calibration_vector(&self, line: u32) -> Option<usize> {
        self.azimuth_lines.binary_search(&line).ok().or_else(|| {
...
impl CompactTimingData {
    fn new() -> Self {
        Self {
            ...
            dc_t0: Vec::new(),
            fm_t0: Vec::new(),
            dc_range_times: Vec::new(),
            fm_range_times: Vec::new(),
        }
    }
}
...
mod tests {
    ...
    fn test_calibration_data_bounds() {
        ...
        cal_data.range_pixels = vec![vec![0, 100, 200], vec![50, 150, 250], vec![25, 125, 225]];
