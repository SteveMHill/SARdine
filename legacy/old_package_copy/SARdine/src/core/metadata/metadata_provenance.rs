...
use crate::core::quality::QualityConfig;
use crate::types::*;
...
"#,
        ...
        xml.push_str(&format!(
            "      <orbit_file_type>{}</orbit_file_type>\n",
        ...
        xml.push_str(&format!(
            "      <overall_quality_score>{:.3}</overall_quality_score>\n",
            self.metadata.quality_metrics.overall_quality_score
        ));
        xml.push_str(&format!(
            "      <valid_pixel_percentage>{:.1}</valid_pixel_percentage>\n",
            self.metadata.quality_metrics.valid_pixel_percentage
        ));
        xml.push_str(&format!(
            "      <mean_snr_db>{:.1}</mean_snr_db>\n",
    ...
    pub fn save_to_file(&self, output_path: &str, format: &str) -> SarResult<()> {
        ...
        std::fs::write(output_path, content).map_err(|e| SarError::Io(e))?;
    ...
    pub fn generate_processing_summary(&self) -> String {
        ...
        summary.push_str("\nUNCERTAINTY ESTIMATES:\n");
        summary.push_str(&format!(
            "  Total Radiometric Uncertainty: ±{:.2} dB\n",
            self.metadata
                .uncertainty_estimates
                .total_radiometric_uncertainty
        ));
        summary.push_str(&format!(
            "  Total Geometric Uncertainty: ±{:.1} m\n",
            self.metadata
                .uncertainty_estimates
                .total_geometric_uncertainty
        ));
