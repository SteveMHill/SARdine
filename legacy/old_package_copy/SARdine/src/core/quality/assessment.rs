    ...
    fn assess_snr(
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                // Apply SNR threshold
                assessment.snr_mask[[i, j]] =
                    snr_db >= config.snr_threshold_db && gamma0_val.is_finite();
            }
        }
    ...
    fn compute_pixel_quality_scores(
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                if config.enable_snr_masking {
                    let snr_normalized = ((assessment.snr_map[[i, j]] - config.snr_threshold_db)
                        / config.snr_normalization_factor)
                        .max(0.0)
                        .min(1.0);
                ...
                if config.enable_geometric_masking {
                    ...
                    let lia_score = if lia >= config.min_local_incidence_angle
                        && lia <= config.max_local_incidence_angle
                    {
                        // Use configurable optimal incidence angle
                        1.0 - ((lia - config.optimal_incidence_angle).abs()
                            / config.optimal_incidence_angle)
                            .min(1.0)
                    } else {
                        0.0
                    };

                    let foreshortening_score =
                        assessment.foreshortening_factors[[i, j]].max(0.0).min(1.0);

                    quality_score +=
                        config.geometric_weight * (lia_score + foreshortening_score) / 2.0;
                ...
                let texture_score = (1.0
                    - (assessment.texture_measures[[i, j]] / config.texture_variance_threshold)
                        .min(1.0))
                .max(0.0);
    ...
    fn combine_quality_masks(
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                if config.enable_pixel_scoring {
                    is_valid &=
                        assessment.pixel_quality_scores[[i, j]] >= config.quality_score_threshold;
                }
