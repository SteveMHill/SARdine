...
use crate::core::geometry::type_safe_units::{
    Degrees, Hertz, Meters, MetersPerSecond, Radians, Seconds,
};
...
impl GeometryF64 {
    ...
    pub fn spherical_to_cartesian(radius: Meters, azimuth: Radians, elevation: Radians) -> Point3D {
        let r = radius.value();
    ...
    pub fn cartesian_to_spherical(point: Point3D) -> (Meters, Radians, Radians) {
        let radius =
            Meters::new((point.x * point.x + point.y * point.y + point.z * point.z).sqrt());
        let azimuth = Radians::new(point.y.atan2(point.x));
        let elevation = Radians::new(
            point
                .z
...
impl DopplerF64 {
    ...
    pub fn velocity_from_frequency(doppler_freq: Hertz, wavelength: Meters) -> MetersPerSecond {
    ...
    pub fn doppler_bandwidth(prf: Hertz, processed_bandwidth_fraction: f64) -> Hertz {
        Hertz::new(prf.value() * processed_bandwidth_fraction)
    }

    /// Calculate theoretical resolution in azimuth
    pub fn azimuth_resolution(velocity: MetersPerSecond, doppler_bandwidth: Hertz) -> Meters {
        let v = velocity.value();
...
impl NewtonRaphsonF64 {
    ...
    pub fn solve_1d<F: NRFunction>(&self, function: &F, initial_guess: f64) -> SarResult<f64> {
        ...
        Err(SarError::NumericalError(format!(
    ...
    pub fn solve_nd<F: NRFunctionND>(
        ...
        Err(SarError::NumericalError(format!(
            "Multi-dimensional Newton-Raphson failed to converge after {} iterations",
            self.max_iterations
        )))
    }
...
impl NRFunctionND for RangeDopplerToGeo {
    ...
    fn evaluate(&self, coords: &[f64]) -> Vec<f64> {
        ...
        let los_unit = GeometryF64::normalize(los_vector).unwrap_or(Point3D {
            x: 0.0,
            y: 0.0,
            z: 1.0,
        });
...
impl RangeDopplerToGeo {
    ...
    fn measured_range(&self) -> Meters {
        Meters::new(850000.0)
    }
    fn measured_doppler(&self) -> Hertz {
        Hertz::new(-7193.46)
    }
    fn reference_elevation(&self) -> Meters {
        Meters::new(0.0)
    }
}
        ...
        for &value in input {
            if value.is_finite() {
                ...
                if (value - f32_val as f64).abs() / value.abs() > 1e-6 {
                    eprintln!(
                        "Warning: Significant precision loss converting {} to f32",
                        value
                    );
                }
                output.push(f32_val);
                ...
                return Err(SarError::NumericalError(format!(
                    "Non-finite value in f64 to f32 conversion: {}",
                    value
                )));
            }
        }
    ...
    pub fn validate_f32_conversion_range(values: &[f64]) -> SarResult<()> {
        for &value in values {
            ...
            if value != 0.0 && value.abs() < f32::MIN_POSITIVE as f64 {
                return Err(SarError::NumericalError(format!(
                    "Value {} below f32 minimum positive",
                    value
                )));
            }
        }
