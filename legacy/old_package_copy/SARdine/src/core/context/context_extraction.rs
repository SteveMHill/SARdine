...
fn extract_product_info(annotation: &AnnotationRoot) -> SarResult<ProductInfo> {
    let ads_header = annotation
        .ads_header
        .as_ref()
        .ok_or_else(|| crate::types::SarError::Metadata("Missing ads_header".to_string()))?;

    let image_info = annotation
        .image_annotation
        .as_ref()
        .and_then(|ia| ia.image_information.as_ref());

    // Extract pass direction from generalAnnotation/productInformation/pass
    // This is REQUIRED for scientific processing - needed for terrain correction
    let pass_direction = annotation
        .general_annotation
        .as_ref()
        .and_then(|ga| ga.product_information.as_ref())
        .and_then(|pi| pi.pass.clone())
        .ok_or_else(|| {
            crate::types::SarError::Metadata(
                "Missing pass direction in generalAnnotation/productInformation/pass".to_string(),
            )
        })?;

    // Parse start time - MUST be present and valid
    let start_time = ads_header
        .start_time
        .as_ref()
        .and_then(|s| crate::io::annotation::parse_time_robust(s))
        .ok_or_else(|| {
            crate::types::SarError::Metadata("Missing or invalid product start time".to_string())
        })?;

    // Parse stop time - MUST be present and valid
    let stop_time = ads_header
        .stop_time
        .as_ref()
        .and_then(|s| crate::io::annotation::parse_time_robust(s))
        .ok_or_else(|| {
            crate::types::SarError::Metadata("Missing or invalid product stop time".to_string())
        })?;

    // Validate time ordering
    if stop_time < start_time {
        return Err(crate::types::SarError::Metadata(format!(
            "Product stop time {:?} is before start time {:?}",
    ...
    Ok(ProductInfo {
        ...
        acquisition_mode: ads_header
            .mode
            .clone()
            .unwrap_or_else(|| "UNKNOWN".to_string()),
        product_type: ads_header
            .product_type
            .clone()
            .unwrap_or_else(|| "UNKNOWN".to_string()),
        polarisation: ads_header
            .polarisation
            .clone()
            .unwrap_or_else(|| "UNKNOWN".to_string()),
        ...
        start_time,
        stop_time,
        pass_direction, // Extracted from generalAnnotation/productInformation/pass
    })
}
...
fn extract_timing_context(
    ...
    let product_start_time_abs = {
        let start_opt = image_info.product_first_line_utc_time.clone().or_else(|| {
            annotation
                .ads_header
                .as_ref()
                .and_then(|h| h.start_time.clone())
        });
        let dt = start_opt
            .and_then(|s| crate::io::annotation::parse_time_robust(&s))
            .ok_or_else(|| {
                crate::types::SarError::Metadata("Missing product start time".to_string())
            })?;
    ...
    let product_stop_time_abs = {
        let stop_opt = image_info.product_last_line_utc_time.clone().or_else(|| {
            annotation
                .ads_header
                .as_ref()
                .and_then(|h| h.stop_time.clone())
        });
        let dt = stop_opt
            .and_then(|s| crate::io::annotation::parse_time_robust(&s))
            .ok_or_else(|| {
                crate::types::SarError::Metadata("Missing product stop time".to_string())
            })?;
        (dt.timestamp() as f64) + (dt.timestamp_subsec_nanos() as f64) * 1e-9
    };
    ...
    let prf = annotation
        ...
        .map_err(|e| crate::types::SarError::Metadata(format!("PRF extraction failed: {}", e)))?;

    // Get azimuth time interval (CRITICAL - must be from annotation, not computed!)
    let azimuth_time_interval = image_info.azimuth_time_interval.ok_or_else(|| {
        crate::types::SarError::Metadata(
...
fn extract_radar_parameters(annotation: &AnnotationRoot) -> SarResult<RadarParameters> {
    let (range_ps, az_ps) = annotation
        .get_pixel_spacing()
        .ok_or_else(|| crate::types::SarError::Metadata("Missing pixel spacing".to_string()))?;

    let carrier_frequency = annotation
        .get_radar_frequency_hz()
    ...
    // Extract range processing bandwidth from imageAnnotation/processingInformation/swathProcParamsList
    // This is the chirp bandwidth used for range compression
    let range_bandwidth = annotation
        .image_annotation
        .as_ref()
        .and_then(|ia| ia.processing_information.as_ref())
        .and_then(|pi| pi.swath_proc_params_list.as_ref())
        .and_then(|spl| spl.swath_proc_params.as_ref())
        .and_then(|params| params.first())
        .and_then(|sp| sp.range_processing.as_ref())
        .map(|rp| rp.processing_bandwidth);
    ...
    Ok(RadarParameters {
        ...
        speed_of_light,
        chirp_bandwidth: range_bandwidth, // From rangeProcessing/processingBandwidth
        range_bandwidth, // Same value - chirp bandwidth = range processing bandwidth
    })
}
...
fn extract_geometry_context(
    ...
    let ellipsoid_incidence_angle_rad = annotation
        .image_annotation
        .as_ref()
        .and_then(|ia| ia.image_information.as_ref())
...
fn get_scene_center(annotation: &AnnotationRoot) -> (Option<f64>, Option<f64>) {
    ...
    if let Some(geo_grid) = annotation
        .geolocation_grid
        .as_ref()
...
fn extract_burst_context(annotation: &AnnotationRoot) -> SarResult<Option<BurstContext>> {
    ...
    // Get lines per burst
    let lines_per_burst = swath_timing.lines_per_burst.ok_or_else(|| {
        crate::types::SarError::Metadata("Missing linesPerBurst in SwathTiming".to_string())
    })? as f64;

    // Extract azimuth FM rate from generalAnnotation/azimuthFmRateList
    // Use the first polynomial coefficient (c0) at mid-swath as representative value
    let azimuth_fm_rate = annotation
        .general_annotation
        .as_ref()
        .and_then(|ga| ga.azimuth_fm_rate_list.as_ref())
        .and_then(|frl| frl.azimuth_fm_rates.as_ref())
        .and_then(|rates| rates.first())
        .map(|rate| {
            // azimuthFmRatePolynomial has coefficients [c0, c1, c2] for k_a(tSR) = c0 + c1*(tSR-t0) + c2*(tSR-t0)^2
            // c0 is the FM rate at t0 in Hz/s
            rate.azimuth_fm_rate_polynomial
                .first()
                .copied()
                .unwrap_or(0.0)
        })
    ...
    // Extract azimuth steering rate from generalAnnotation/productInformation/azimuthSteeringRate
    let azimuth_steering_rate = annotation
        .general_annotation
        .as_ref()
        .and_then(|ga| ga.product_information.as_ref())
        .and_then(|pi| pi.azimuth_steering_rate)
        .unwrap_or(0.0);

    // Extract Doppler centroid from dc_estimate_list
    // Use the first polynomial coefficient (c0) as representative value at mid-swath
    let doppler_centroid = annotation
        .general_annotation
        .as_ref()
        .and_then(|ga| ga.dc_estimate_list.as_ref())
        .and_then(|dcl| dcl.dc_estimates.as_ref())
        .and_then(|ests| ests.first())
        .map(|est| est.data_dc_polynomial.first().copied().unwrap_or(0.0))
        .or_else(|| {
            // Fallback to doppler_centroid field
            annotation
                .doppler_centroid
                .as_ref()
                .and_then(|dc| dc.dc_estimate_list.as_ref())
    ...
    for (idx, burst) in bursts_data.iter().enumerate() {
        ...
        let sensing_start = burst
            .azimuth_time
            .as_ref()
            .and_then(|s| crate::io::annotation::parse_time_robust(s))
            .ok_or_else(|| {
                crate::types::SarError::Metadata(format!(
                    "Missing or invalid azimuth_time for burst {}",
                    idx
                ))
            })?;
        ...
        let sensing_stop =
            sensing_start + chrono::Duration::milliseconds((burst_duration_s * 1000.0) as i64);
        ...
        bursts.push(BurstMetadata {
            ...
            sensing_start,
            sensing_stop,
            azimuth_fm_rate,       // Extracted from azimuthFmRateList
            azimuth_steering_rate, // Extracted from productInformation
            doppler_centroid,      // Extracted from dcEstimateList
        });
    }
...
mod tests {
    ...
    fn test_heading_estimation() {
        ...
        let orbit_data = OrbitData {
            ...
            state_vectors: vec![crate::types::StateVector {
                time: fixed_time,
