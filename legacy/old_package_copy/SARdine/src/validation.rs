...
impl ValidationGateway {
    ...
    /// # Usage
    /// ```rust
    /// use sardine::validation::{ValidationConfig, ValidationGateway};
    ///
    /// let config = ValidationConfig::new()
    ///     .with_strictness(false)
    ///     .with_cache_enabled(true);
    /// let _gateway = ValidationGateway::with_config(config);
    /// ```
    ...
    fn validate_annotation_structure(
        ...
        // FIXED: Use tolerance-based comparison instead of exact equality
        // to avoid false positives from legitimate annotation values.
        // Also check for obviously rounded values (no decimal precision)
        // that suggest hardcoding rather than real annotation extraction.
        const SUSPICIOUS_TOLERANCE: f64 = 1e-6;
        let is_suspicious_range = (range_spacing - 10.0).abs() < SUSPICIOUS_TOLERANCE
            || (range_spacing - 2.3).abs() < SUSPICIOUS_TOLERANCE;
        let is_suspicious_azimuth = (azimuth_spacing - 10.0).abs() < SUSPICIOUS_TOLERANCE
            || (azimuth_spacing - 2.3).abs() < SUSPICIOUS_TOLERANCE
            || (azimuth_spacing - 14.0).abs() < SUSPICIOUS_TOLERANCE;

        // Only flag as suspicious if BOTH values look hardcoded (round numbers)
        // Single suspicious value might just be coincidental
        if is_suspicious_range && is_suspicious_azimuth {
            warnings.push(
    ...
    fn detect_hardcoded_parameters(
        ...
        // FIXED: Use tolerance-based comparison to avoid false positives
        const SUSPICIOUS_TOLERANCE: f64 = 1e-6;
        let is_suspicious_range = (range_spacing - 2.3).abs() < SUSPICIOUS_TOLERANCE
            || (range_spacing - 10.0).abs() < SUSPICIOUS_TOLERANCE;
        let is_suspicious_azimuth = (azimuth_spacing - 10.0).abs() < SUSPICIOUS_TOLERANCE
            || (azimuth_spacing - 2.3).abs() < SUSPICIOUS_TOLERANCE
            || (azimuth_spacing - 14.0).abs() < SUSPICIOUS_TOLERANCE;

        // Only flag if both look hardcoded
        if is_suspicious_range && is_suspicious_azimuth {
...
mod tests {
    ...
    fn test_hardcoded_wavelength_detection() {
        ...
        // Precise Sentinel-1 annotation wavelength should be accepted
        assert!(validator
            .validate_wavelength(0.055_465_763, "sentinel-1-annotation")
            .is_ok());
    }
    ...
    fn test_hardcoded_spacing_detection() {
        ...
        assert!(validator
            .validate_pixel_spacing(2.8767, 10.0, "test")
            .is_err());
        assert!(validator
            .validate_pixel_spacing(10.0, 14.0, "test")
            .is_err());
        assert!(validator
            .validate_pixel_spacing(10.0, 14.1, "test")
            .is_err());

        // High-precision Sentinel-1 annotation spacing should be accepted
        assert!(validator
            .validate_pixel_spacing(2.329_562, 13.943_035, "sentinel-1-annotation")
            .is_ok());
    }
    ...
    fn test_valid_parameters() {
        ...
        let frequency_hz = 5.405e9; // Sentinel-1 C-band frequency from annotation
        let wavelength_m = SPEED_OF_LIGHT_M_S as f64 / frequency_hz;
        let range_spacing_m = 2.329_562; // Native pixel spacing from annotation metadata
        let azimuth_spacing_m = 13.943_035; // Native azimuth spacing from annotation metadata
        let prf_hz = 1_718.0; // Valid PRF within mission envelope
