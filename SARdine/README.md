...
### Calibration + Merge GeoTIFFs (pre-geocode)

Run just the deburst → calibration → TOPSAR merge stages and capture their outputs as GeoTIFFs:

```
python -m sardine.partial_pipeline S1A_IW_SLC__*.SAFE ./partial_output --polarization VV
```

This produces one GeoTIFF per calibrated subswath (before multilooking) plus a merged intensity GeoTIFF and a JSON summary describing timings and coverage statistics. Use this when you need fast access to calibrated intensity rasters without running the full geocoding phase.
