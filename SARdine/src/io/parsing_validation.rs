...
impl ParsingValidator {
    ...
    pub fn validate_parsing_equivalence(
        ...
        Ok(ValidationResult {
            equivalent: true,
            details: "Parsing validation infrastructure ready - full implementation pending"
                .to_string(),
        })
    }
}
    ...
    for (i, burst) in bursts.iter().enumerate() {
        ...
        if steering_diff > 0.0001 {
            log::warn!(
                ...
                i, burst.azimuth_steering_rate, reference_steering
            );
        }
    }
