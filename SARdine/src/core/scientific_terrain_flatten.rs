...
/// 
/// Surface normals are computed from DEM gradients using the standard approach:
/// 1. Calculate gradients: p = ∂z/∂x, q = ∂z/∂y (using central differences)
...
impl TerrainFlattener {
    ...
    /// Compute surface normal from DEM gradients using central differences
    /// 
    ...
    pub fn compute_surface_normal(
        ...
        if rows < 3 || cols < 3 {
            return Err(SarError::Processing("DEM too small for gradient computation (minimum 3x3)".to_string()));
        }
                ...
                .map(|row_chunk| {
                    ...
                    for &i in row_chunk {
                        for j in 1..cols-1 {
                            ...
                            let p = (dem[[i, j+1]] - dem[[i, j-1]]) / (2.0 * dx);  // ∂z/∂x
                            let q = (dem[[i+1, j]] - dem[[i-1, j]]) / (2.0 * dy);  // ∂z/∂y
    ...
    pub fn process(
        ...
        log::debug!("Computing local incidence angles: θ_local = angle between normal and look vector");
        ...
        log::info!("  Valid pixels: {} ({:.1}%)", valid_pixels, 100.0 * valid_pixels as f32 / total_pixels as f32);
        log::info!("  Shadow pixels: {} ({:.1}%)", shadow_pixels, 100.0 * shadow_pixels as f32 / total_pixels as f32);
        ...
        if !finite_gamma0.is_empty() {
            ...
            let max_gamma0 = finite_gamma0.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b));
            log::info!("  γ⁰ range: {:.6} to {:.6} (linear units)", min_gamma0, max_gamma0);
            ...
            log::info!("  γ⁰ range: {:.1} to {:.1} dB (for reference only)", min_db, max_db);
        }
...
pub fn terrain_flatten_orbital(
    ...
    let params = TerrainFlatteningParams::orbital(
        dem_pixel_spacing.unwrap_or(30.0)
    );
...
mod tests {
    ...
    fn test_simple_terrain_flattening() {
        ...
        let result = terrain_flatten(&sigma0, &dem, Some(0.0), Some(35.0_f32.to_radians()), Some(30.0));
        assert!(result.is_ok());
...
mod tests {
    ...
    fn test_surface_normal_computation() {
        ...
        dem[[0, 0]] = 0.0; dem[[0, 1]] = 10.0; dem[[0, 2]] = 20.0;
        dem[[1, 0]] = 5.0; dem[[1, 1]] = 15.0; dem[[1, 2]] = 25.0;
        dem[[2, 0]] = 10.0; dem[[2, 1]] = 20.0; dem[[2, 2]] = 30.0;
        ...
        for i in 0..3 {
            for j in 0..3 {
                ...
                assert!((magnitude - 1.0).abs() < 1e-5, "Normal should be unit vector");
            }
        }
    }
    ...
    fn test_look_vector_computation() {
        ...
        assert!((look_vector[0] - 0.707).abs() < 0.01, "Look vector x component");
        assert!(look_vector[1].abs() < 0.01, "Look vector y component should be near zero");
        assert!((look_vector[2] + 0.707).abs() < 0.01, "Look vector z component");
        
        // Look vector should be unit vector
        let magnitude = (look_vector[0]*look_vector[0] + look_vector[1]*look_vector[1] + look_vector[2]*look_vector[2]).sqrt();
        assert!((magnitude - 1.0).abs() < 1e-5, "Look vector should be unit vector");
    }
}
    ...
    fn test_steep_slope_safeguard() {
        ...
        let mut dem = Array2::<f32>::zeros((3, 3));
        dem[[0, 0]] = 0.0; dem[[0, 1]] = 0.0; dem[[0, 2]] = 0.0;
        dem[[1, 0]] = 100.0; dem[[1, 1]] = 100.0; dem[[1, 2]] = 100.0; // Very steep
        dem[[2, 0]] = 200.0; dem[[2, 1]] = 200.0; dem[[2, 2]] = 200.0;
        ...
        assert!(gamma0.iter().all(|&x| x.is_finite()));
        assert!(gamma0.iter().all(|&x| x <= 10.0), "Steep slope safeguard should limit amplification");
    }
}