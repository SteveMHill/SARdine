...
use crate::core::type_safe_units::{Seconds, Hertz};
...
pub enum MockTimeVariation {
    ...
    Linear { slope_dc: f64, slope_fm: f64, reference_time: Seconds },
    /// Sinusoidal variation for testing periodic effects
    Sinusoidal { amplitude_dc: f64, amplitude_fm: f64, period: Seconds, phase: f64 },
...
impl MockDcFmProvider {
    ...
    fn get_dc_at_time(&self, az_time: Seconds) -> Hertz {
        ...
        match &self.time_variation {
            None => self.dc_value,
            Some(MockTimeVariation::Linear { slope_dc, reference_time, .. }) => {
                let dt = az_time.value() - reference_time.value();
                Hertz::new(base_dc + slope_dc * dt)
            }
            Some(MockTimeVariation::Sinusoidal { amplitude_dc, period, phase, .. }) => {
                let phase_rad = 2.0 * std::f64::consts::PI * az_time.value() / period.value() + phase;
    ...
    fn get_fm_rate_at_time(&self, az_time: Seconds) -> f64 {
        ...
        match &self.time_variation {
            None => base_fm,
            Some(MockTimeVariation::Linear { slope_fm, reference_time, .. }) => {
                let dt = az_time.value() - reference_time.value();
                base_fm + slope_fm * dt
            }
            Some(MockTimeVariation::Sinusoidal { amplitude_fm, period, phase, .. }) => {
                let phase_rad = 2.0 * std::f64::consts::PI * az_time.value() / period.value() + phase;
...
impl DcFmProviderFactory {
    ...
    pub fn from_polynomials(
        ...
        let provider = PolynomialDcFmProvider::new(
            dc_polynomials,
            fm_polynomials,
            burst_time_ranges,
        )?;
...
mod tests {
    ...
    fn test_polynomial_evaluation() {
        ...
        let result = PolynomialDcFmProvider::evaluate_polynomial(
