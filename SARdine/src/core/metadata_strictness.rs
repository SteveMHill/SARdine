...
impl StrictMetadataValidator {
    ...
    pub fn validate_strict(metadata: &SarMetadata) -> SarResult<StrictValidationResult> {
        ...
        if metadata.range_sampling_rate.is_none() {
            ...
            if rsr <= 0.0 || !rsr.is_finite() {
                ...
                warnings.push(format!("Range sampling rate {} Hz is outside typical SAR range (1-1000 MHz)", rsr));
            }
        }
        ...
        if matches!(metadata.acquisition_mode, crate::types::AcquisitionMode::IW | crate::types::AcquisitionMode::EW) {
        ...
        for (swath_id, subswath) in sub_swaths {
            ...
            if let Some(prf) = subswath.prf_hz {
                if !prf.is_finite() || prf <= 0.0 {
                    errors.push(format!("CRITICAL: Invalid PRF for Doppler calculation in {}: {}", swath_id, prf));
                } else if let Some(dc_poly) = &subswath.dc_polynomial {
                ...
                errors.push(format!("CRITICAL: PRF missing for Doppler validation in {}", swath_id));
            }
        }
    ...
    pub fn validate_beta_variability_with_threshold(beta_values: &[f64], threshold: f64) -> BetaVariabilityResult {
        ...
        let max_beta = valid_values.iter().copied().fold(f64::NEG_INFINITY, |a, b| a.max(b));
    ...
    fn detect_dB_values_statistical(lut_name: &str, values: &[f64], conversions_needed: &mut Vec<String>) {
        ...
        let positive_values: Vec<f64> = finite_values.iter().copied().filter(|&v| v > 0.0).collect();
        if !positive_values.is_empty() {
            let log_range = positive_values.iter()
                .map(|&v| v.log10())
        ...
        if likely_dB && !conversions_needed.contains(&lut_name.to_string()) {
            log::warn!("🔍 Statistical dB detection for {}: median={:.3}, IQR={:.3}, range=[{:.3}, {:.3}]", 
                      lut_name, median, iqr, min_val, max_val);
            conversions_needed.push(lut_name.to_string());
        }
    }
    ...
    fn check_seam_power_consistency(
        ...
        if neighbor_mean_power < 1e-10 {
            log::warn!("⚠️ Neighbor mean power near zero ({:.2e}), cannot compute deviation", neighbor_mean_power);
            return None;
        }
    ...
    pub fn validate_coverage_with_threshold(uncovered_mask: &Array2<u8>, threshold_percent: f64) -> CoverageResult {
        ...
        if !is_valid {
            ...
            log::info!("✅ Coverage validation passed: {:.3}% uncovered (threshold: {:.2}%)", 
                      uncovered_percent, threshold_percent);
        }
