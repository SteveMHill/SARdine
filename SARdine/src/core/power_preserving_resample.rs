...
impl PowerPreservingResampler {
    ...
    pub fn resample_azimuth_line(
        ...
        if n_in != input_times.len() {
            return Err(SarError::DataProcessingError(
                format!("Input size mismatch: {} samples vs {} times", n_in, input_times.len()),
            ));
        }
    ...
    pub fn deramp_signal(
        ...
        for i in 0..n {
            ...
            let phase = 2.0 * std::f64::consts::PI * dc_hz * t
        ...
        for (out_idx, &t_out) in output_times.iter().enumerate() {
            ...
            for tap_offset in 0..taps {
                ...
                if input_idx >= 0 && (input_idx as usize) < n_in {
                    ...
                    let window_normalized = (tap_offset as f64 - half_taps as f64) / (half_taps as f64);
...
pub fn calculate_power_ratio(
    input: ArrayView2<SarComplex>,
    output: ArrayView2<SarComplex>,
) -> f32 {
    let input_power: f32 = input.iter().map(|c| c.norm_sqr()).sum();
    let output_power: f32 = output.iter().map(|c| c.norm_sqr()).sum();
...
mod tests {
    ...
    fn test_power_preservation_resample() {
        ...
        assert!(ratio > 0.99 && ratio < 1.01, "Power not preserved: ratio = {}", ratio);
    }
    ...
    fn test_sinc_kernel_interpolation() {
        ...
        assert!(fft_ratio > 0.99 && fft_ratio < 1.01, 
            "FFT power not preserved: ratio = {:.4} (expected 0.99-1.01)", fft_ratio);
        ...
        assert!(max_diff < 1.0, 
            "FFT and sinc results too different: max_diff = {:.6} (expected < 1.0)", max_diff);
    }
