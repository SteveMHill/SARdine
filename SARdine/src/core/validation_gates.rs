...
impl ValidationResult {
    ...
    pub fn fail(test_name: &str, measured: f64, threshold: f64, tolerance: f64, message: &str) -> Self {
        Self {
            passed: false,
            test_name: test_name.to_string(),
            measured_value: measured,
...
impl ValidationGates {
    ...
    pub fn validate_power_preservation(
        ...
        if !self.enable_power_preservation {
            return Ok(ValidationResult::pass(
                "Power Preservation", 0.0, 0.0, "Validation disabled"
            ));
        }
        ...
        for &(row_start, row_end, col_start, col_end) in overlap_regions {
            for row in row_start..row_end {
                for col in col_start..col_end {
                    if row < pre_deburst_power.nrows() && col < pre_deburst_power.ncols() &&
                       row < post_deburst_power.nrows() && col < post_deburst_power.ncols() {
    ...
    let power_ratio = post_mean_power / (pre_mean_power + eps);
        ...
        info!(
            ...
            pre_mean_power, post_mean_power, power_ratio, power_error * 100.0
        );
    ...
    pub fn validate_uncovered_pixels(
        ...
        if !self.enable_uncovered_pixel_check {
            return Ok(ValidationResult::pass(
                "Uncovered Pixels", 0.0, 0.0, "Validation disabled"
            ));
        }
        ...
        for (i, &pixel_value) in debursted_image.iter().enumerate() {
            let is_uncovered = pixel_value.is_nan() || 
                              pixel_value == 0.0 || 
                              pixel_value.is_infinite();
        ...
        info!(
            ...
            uncovered_pixels, total_pixels, uncovered_rate * 100.0
        );
    ...
    pub fn validate_nesz_compliance(
        ...
        if !self.enable_nesz_check {
            return Ok(ValidationResult::pass(
                "NESZ Compliance", 0.0, 0.0, "Validation disabled"
            ));
        }
        // Heuristic dB domain assertions (sigma0 and NESZ LUT should be dB values here)
        if !sigma0_image.is_empty() {
            let suspicious_sigma = sigma0_image.iter().take(32).filter(|v| v.is_finite()).any(|v| *v > 100.0);
            if suspicious_sigma {
    ...
    pub fn validate_point_target_analysis(
        ...
        if !self.enable_pta_check {
            results.push(ValidationResult::pass(
                "Point Target Analysis", 0.0, 0.0, "PTA validation disabled"
            ));
            return Ok(results);
        }
        ...
        if pslr_db <= self.pslr_threshold_db {
            results.push(ValidationResult::pass(
                ...
                &format!("PSLR {:.2} dB meets requirement (<{:.1} dB)", pslr_db, self.pslr_threshold_db)
            ));
        } else {
            results.push(ValidationResult::fail(
                ...
                self.pslr_threshold_db,
                1.0,
                &format!("PSLR {:.2} dB exceeds {:.1} dB threshold", pslr_db, self.pslr_threshold_db)
            ).with_recommendations(vec![
        ...
        if islr_db <= self.islr_threshold_db {
            results.push(ValidationResult::pass(
                ...
                &format!("ISLR {:.2} dB meets requirement (<{:.1} dB)", islr_db, self.islr_threshold_db)
            ));
        } else {
            results.push(ValidationResult::fail(
                "ISLR",
                ...
                self.islr_threshold_db,
                1.0,
                &format!("ISLR {:.2} dB exceeds {:.1} dB threshold", islr_db, self.islr_threshold_db)
            ).with_recommendations(vec![
        ...
        if irw_meters <= self.irw_threshold_m {
            results.push(ValidationResult::pass(
                ...
                &format!("IRW {:.1}m meets requirement (<{:.1}m)", irw_meters, self.irw_threshold_m)
            ));
        } else {
            results.push(ValidationResult::fail(
                "IRW",
                ...
                self.irw_threshold_m,
                2.0,
                &format!("IRW {:.1}m exceeds {:.1}m threshold", irw_meters, self.irw_threshold_m)
            ).with_recommendations(vec![
    ...
    pub fn validate_geometry_accuracy(
        ...
        if !self.enable_geometry_check {
            return Ok(ValidationResult::pass(
                "Geometry Accuracy", 0.0, 0.0, "Validation disabled"
            ));
        }
        ...
        let mean_azimuth_error: f64 = azimuth_errors.iter().sum::<f64>() / azimuth_errors.len() as f64;
        ...
        info!(
            ...
            ce90_error, mean_azimuth_error, mean_range_error, measured_positions.len()
        );
        ...
        if ce90_error <= self.geometry_ce90_threshold {
            Ok(ValidationResult::pass(
                ...
                &format!(
                    ...
                    ce90_error, self.geometry_ce90_threshold, measured_positions.len()
                )
            ))
        } else {
            ...
            if mean_azimuth_error.abs() > mean_range_error.abs() {
                recommendations.push("Azimuth error dominant - check timing/PRF parameters".to_string());
            } else {
                recommendations.push("Range error dominant - check range sampling/delay".to_string());
            }
    ...
    pub fn validate_lut_domain_bounds(
        ...
        if !self.enable_lut_domain_check {
            return Ok(ValidationResult::pass(
                "LUT Domain Check", 0.0, 0.0, "Validation disabled"
            ));
        }
        ...
        if out_of_bounds_count == 0 {
            ...
            error!(
                ...
                operation_name, out_of_bounds_count, total_indices, violation_rate * 100.0
            );
            
            if max_line_violation > 0 {
                error!("   Max line violation: {} >= {}", max_line_violation, max_lines);
            }
            if max_pixel_violation > 0 {
                error!("   Max pixel violation: {} >= {}", max_pixel_violation, max_pixels);
            }
    ...
    pub fn run_validation_suite(
        ...
        if let (Some(pre), Some(post), Some(regions)) = (pre_deburst_power, post_deburst_power, overlap_regions) {
            match self.validate_power_preservation(pre, post, regions) {
                ...
                Err(e) => {
                    ...
                    all_results.push(ValidationResult::fail(
                        "Power Preservation", 0.0, 0.0, 0.0, &format!("Validation error: {}", e)
                    ));
                }
            }
        }
        ...
        if let Some(image) = debursted_image {
            match self.validate_uncovered_pixels(image, None) {
                ...
                Err(e) => {
                    ...
                    all_results.push(ValidationResult::fail(
                        "Uncovered Pixels", 0.0, 0.0, 0.0, &format!("Validation error: {}", e)
                    ));
                }
            }
        }
        ...
        if let (Some(sigma0), Some(nesz)) = (sigma0_image, nesz_lut) {
            match self.validate_nesz_compliance(sigma0, 10.0, nesz, None) {
                ...
                Err(e) => {
                    ...
                    all_results.push(ValidationResult::fail(
                        "NESZ Compliance", 0.0, 0.0, 0.0, &format!("Validation error: {}", e)
                    ));
                }
            }
        }
        ...
        if let Some((pslr, islr, irw, radio_err)) = pta_metrics {
            match self.validate_point_target_analysis(pslr, islr, irw, radio_err) {
                ...
                Err(e) => {
                    ...
                    all_results.push(ValidationResult::fail(
                        "Point Target Analysis", 0.0, 0.0, 0.0, &format!("Validation error: {}", e)
                    ));
                }
            }
        }
        ...
        if let Some((measured, reference)) = geometry_points {
            match self.validate_geometry_accuracy(measured, reference, "WGS84") {
                ...
                Err(e) => {
                    ...
                    all_results.push(ValidationResult::fail(
                        "Geometry Accuracy", 0.0, 0.0, 0.0, &format!("Validation error: {}", e)
                    ));
                }
            }
        }
        ...
        if passed_count == total_count {
            ...
            warn!(
                ...
                passed_count, total_count, total_count - passed_count
            );
        }
...
impl ValidationSummary {
    ...
    pub fn log_summary(&self) {
        info!("📊 Validation Summary: {}/{} tests passed", self.passed_tests, self.total_tests);
...
mod tests {
    ...
    fn test_power_preservation_validation() {
        ...
        let result = gates.validate_power_preservation(&pre_power, &post_power, &overlap_regions).unwrap();
    ...
    fn test_uncovered_pixel_validation() {
        ...
        for i in 0..3000 { // 3000 uncovered pixels = 0.3%
            let row = i / 1000;
            let col = i % 1000;
            image[(row, col)] = 0.0;
        }
        ...
        let result = gates.validate_uncovered_pixels(&image, None).unwrap();
        assert!(!result.passed, "0.3% uncovered rate should fail (>0.2% threshold)");
    }
    ...
    fn test_lut_domain_validation() {
        ...
        let result = gates.validate_lut_domain_bounds(
            &line_indices, &pixel_indices, 100, 200, "test_operation"
        ).unwrap();
        ...
        let result = gates.validate_lut_domain_bounds(
            &bad_line_indices, &pixel_indices, 100, 200, "test_operation"
        );
