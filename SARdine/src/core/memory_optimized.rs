    ...
    T: Copy + PartialOrd + std::ops::Add<Output = T> + std::ops::Div<Output = T> + From<usize>,
{
    ...
    let mean = sum.map(|s| s / T::from(count));
...
/// 
/// Note: This function uses sequential processing due to Rust's strict borrowing rules.
/// For true parallel processing, consider splitting the array beforehand or using
/// different parallel patterns that don't require mutable borrows.
...
where
    ...
    F: FnMut(ArrayView2<T>, ArrayViewMut2<T>) -> SarResult<()>,
{
    // Due to Rust's borrowing rules, we can't safely split a mutable view across threads
    // without complex synchronization. For now, process sequentially.
    // TODO: Implement using rayon's split_at_mut or unsafe parallel_chunks if needed.
    process_chunked(input, output, chunk_size, operation)
}
...
pub mod inplace_ops {
    ...
    pub fn linear_to_db_inplace_par(
        ...
        if let Some(mask) = mask_invalid {
            ...
            array.axis_iter_mut(Axis(0)).into_par_iter().for_each(
                |mut row| {
