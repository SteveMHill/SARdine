...
impl ParameterValidator {
    ...
    pub fn validate_wavelength(&self, wavelength: f64, source: &str) -> SarResult<()> {
        ...
        for &suspicious in &suspicious_values {
            if (wavelength - suspicious).abs() < 1e-6 {  // Use consistent tolerance
                return Err(SarError::InvalidParameter(format!(
    ...
    pub fn validate_wavelength_with_frequency(
        ...
        if rel_err <= tolerance {
            log::info!("✅ Wavelength {:.9}m consistent with frequency {:.1}Hz (rel_err: {:.2e})", 
                       wavelength, frequency, rel_err);
            return Ok(());
        }
        
        log::warn!("⚠️  Wavelength {:.9}m inconsistent with frequency {:.1}Hz (rel_err: {:.2e} > {:.2e})", 
                  wavelength, frequency, rel_err, tolerance);
    ...
    pub fn validate_frequency_wavelength_consistency(&self, frequency: f64, wavelength: f64) -> SarResult<()> {
        ...
        let tolerance = 1e-6; // More lenient than 1e-9 for real extracted data
...
impl ValidationGateway {
    ...
    pub fn validate_metadata(&self, metadata: &crate::types::SarMetadata) -> SarResult<ValidationReport> {
    ...
    fn perform_comprehensive_validation(&self, metadata: &crate::types::SarMetadata) -> SarResult<ValidationReport> {
        let mut errors = Vec::new();
        let mut warnings = Vec::new();
        let mut scientific_score = 1.0;
        
        // 1. Validate coordinate system
        let coordinate_validation = self.validate_coordinate_system(metadata, &mut errors, &mut warnings, &mut scientific_score)?;
        
        // 2. Validate orbit data
        let orbit_validation = self.validate_orbit_data(metadata, &mut errors, &mut warnings, &mut scientific_score)?;
        
        // 3. Validate calibration data
        let calibration_validation = self.validate_calibration_data(metadata, &mut errors, &mut warnings, &mut scientific_score)?;
        
        // 4. Validate annotation structure
        let annotation_validation = self.validate_annotation_structure(metadata, &mut errors, &mut warnings, &mut scientific_score)?;
        ...
        if self.strict_mode {
            self.apply_strict_validation(metadata, &mut errors, &mut warnings, &mut scientific_score);
        }
    ...
    fn validate_coordinate_system(&self, metadata: &crate::types::SarMetadata, errors: &mut Vec<String>, _warnings: &mut Vec<String>, score: &mut f64) -> SarResult<CoordinateValidation> {
    ...
    fn validate_orbit_data(&self, metadata: &crate::types::SarMetadata, errors: &mut Vec<String>, warnings: &mut Vec<String>, score: &mut f64) -> SarResult<OrbitValidation> {
        ...
        if let Some(ref orbit_data) = metadata.orbit_data {
            ...
            if !orbit_data.state_vectors.is_empty() {
                ...
                let first_time = &orbit_data.state_vectors.first().unwrap().time;
                let last_time = &orbit_data.state_vectors.last().unwrap().time;
                ...
                if coverage_duration >= 10.0 {
                    ...
                    interpolation_quality = (coverage_duration / 60.0).min(1.0); // Better with more coverage
                } else {
                    warnings.push(format!("Limited orbit coverage: {:.1}s (minimum 10s recommended)", coverage_duration));
                    *score -= 0.1;
                }
    ...
    fn validate_calibration_data(&self, metadata: &crate::types::SarMetadata, errors: &mut Vec<String>, warnings: &mut Vec<String>, score: &mut f64) -> SarResult<CalibrationValidation> {
        ...
        if !metadata.sub_swaths.is_empty() {
            ...
            warnings.push("No subswath data found - processing may require calibration data".to_string());
            *score -= 0.15;
        }
        ...
        if let Some(frequency) = metadata.radar_frequency {
            ...
            if frequency >= 5.0e9 && frequency <= 6.0e9 {
                ...
                warnings.push(format!("Unusual radar frequency: {:.2} GHz (expected C-band ~5.4 GHz)", frequency / 1e9));
                *score -= 0.05;
            }
        } else {
    ...
    fn validate_annotation_structure(&self, metadata: &crate::types::SarMetadata, errors: &mut Vec<String>, warnings: &mut Vec<String>, score: &mut f64) -> SarResult<AnnotationValidation> {
        ...
        if azimuth_spacing == 10.0 || range_spacing == 10.0 || 
           azimuth_spacing == 2.3 || range_spacing == 2.3 {
            warnings.push("Suspicious pixel spacing values detected - verify these are from annotation XML".to_string());
    ...
    fn detect_hardcoded_parameters(&self, metadata: &crate::types::SarMetadata, warnings: &mut Vec<String>, score: &mut f64) {
        // Use existing hardcoded detection patterns
        let hardcoded_patterns = vec![
            ("Suspicious frequency", metadata.radar_frequency.map_or(false, |f| (f - 5.405e9).abs() < 1000.0)),
            ("Suspicious pixel spacing", {
                let (range, azimuth) = metadata.pixel_spacing;
                range == 2.3 || azimuth == 10.0 || range == 10.0 || azimuth == 2.3
            }),
        ];
        
        for (pattern_name, is_suspicious) in hardcoded_patterns {
            if is_suspicious {
                warnings.push(format!("Hardcoded value detected: {}", pattern_name));
                *score -= 0.05;
            }
        }
    }
    ...
    fn apply_strict_validation(&self, metadata: &crate::types::SarMetadata, errors: &mut Vec<String>, warnings: &mut Vec<String>, score: &mut f64) {
        ...
        if let Some(ref orbit_data) = metadata.orbit_data {
            if orbit_data.state_vectors.len() < 5 {
                errors.push("STRICT MODE: Insufficient orbit state vectors for accurate interpolation".to_string());
                *score *= 0.5;
            }
        }
        ...
        if metadata.sub_swaths.is_empty() {
            errors.push("STRICT MODE: Missing subswath data required for scientific accuracy".to_string());
            *score *= 0.5;
        }
    }
    ...
    pub fn configure_cache(&self, config: &ValidationConfig) {
        self.clear_cache();
        log::info!("🔧 ValidationGateway cache reconfigured: expiry={}h, enabled={}", 
                  config.cache_expiry_hours, config.cache_enabled);
    }
